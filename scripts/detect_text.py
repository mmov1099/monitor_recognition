...
def create_gui_window(img, img_name, param, height, width, args):
    ...
    def printCoor(event,x,y,flags,param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            print(f'start x:{x}, y:{y} --height:{height},width:{width}--')
            ...
            for i in range(args.num_digits):
                ...
                if args.test or args.make_data:
                    trim_array = cv2.cvtColor(trim_array, cv2.COLOR_BGR2GRAY)
                    ret, trim_array = cv2.threshold(trim_array, args.gray_threshold, 255, cv2.THRESH_BINARY)
                    trim_array=255-trim_array
...
def run_gui(img, img_name, res_file_path, args):
    height = args.height
    width = args.width*args.num_digits
